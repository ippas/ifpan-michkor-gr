...
freq_all_genes %>% 
  ...
  as.data.frame() %>% 
  set_colnames(c("freq_genes", "n_genes")) %>% 
  mutate(propability = n_genes/13252) %>% 
  mutate(cumsum = cumsum(propability)) %>% 
  mutate(rev_cumsum = 1 - cumsum) %>% 
  mutate(freq_genes = ifelse(n_genes < 113, "more_than_12", as.character(freq_genes))) %>% 
  group_by(freq_genes) %>% 
  summarise(n_genes = sum(n_genes), propability = sum(propability)) %>%
  ungroup() %>% 
  mutate(freq_genes = factor(freq_genes, levels = c(as.character(1:13), "more_than_12"))) %>% 
  arrange(freq_genes) %>% 
  mutate(cumsum = cumsum(propability)) 
