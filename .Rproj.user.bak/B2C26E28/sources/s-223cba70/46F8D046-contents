...
filtered_data <- filt_1_gr_database %>%
...
svg("results/figures/gene-database-summary/filtered-1-gene-database-summary/boxplot-up-down.svg", width = 6, height = 12)
...
# 2. Add total sums for occurrences and unique genes
total_counts <- regulation_summary %>%
  dplyr::summarize(
    total_gene_occurrences = sum(gene_occurrences),
    total_unique_genes = sum(unique_genes)
  )

# Display results
print(regulation_summary) %>% t()
print(total_counts)
