...
venn_data <- gene_list_log2ratio_top_50 %>% 
  select(-data) %>% 
  unnest(top_50_genes) %>% 
  ungroup %>% 
...
# svg("results/figures/harmonized-tissue-gene-lists/summary-gene-lists-top50/venn-treatment-type.svg", width = 8, height = 8)
grid.draw(venn_plot)
# dev.off()
