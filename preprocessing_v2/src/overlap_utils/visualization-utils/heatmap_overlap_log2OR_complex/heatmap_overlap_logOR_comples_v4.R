heatmap_overlap_log2OR_complex <- function(
    ...
    # ---- NEW: choose matrix for COLORS by key in data_list[[data_type]]$list ----
    color_key = "log2_odds_ratio_matrix",   # e.g. "chi2_matrix", "p_value_matrix", "number_overlap_matrix"
  ...
  # ---- LABEL MATRIX (fixed: log2or + overlap) ----
  label_mat <- ifelse(!is.na(log2or), sprintf("%.2f (%d)", log2or, overlap), "")
  ...
  # ---- SIGNIFICANCE (fixed: pval + overlap) ----
