heatmap_overlap_log2OR_complex <- function(
    ...
    row_order_original = NULL,   # <=== NOWY ARGUMENT (ręczny porządek wierszy)
    col_order_original = NULL,   # <=== ręczny porządek kolumn
  ...
  # bezpieczne mapowanie: jeśli mapper NULL, to identyczność
  if (!is.null(row_mapper)) {
    mapped_rows <- ifelse(current_rows %in% names(row_mapper),
                          row_mapper[current_rows], current_rows)
  } else {
    mapped_rows <- current_rows
  }
  
  if (!is.null(col_mapper)) {
    mapped_cols <- ifelse(current_cols %in% names(col_mapper),
                          col_mapper[current_cols], current_cols)
  } else {
    mapped_cols <- current_cols
  }
  ...
  # ---- MANUAL ROW ORDER USING ORIGINAL NAMES ----
  # row_order_original podajesz w "oryginalnych nazwach" (przed dopisaniem (n))
  ...
  # ---- MANUAL COLUMN ORDER USING ORIGINAL NAMES ----
  ...
  ht <- ComplexHeatmap::Heatmap(
    ...
    cell_fun = function(j,i,x,y,w,h,fill){
      ...
      grid::grid.text(
        label_mat[i,j], x, y,
        gp = grid::gpar(col=text_col_mat[i,j],
