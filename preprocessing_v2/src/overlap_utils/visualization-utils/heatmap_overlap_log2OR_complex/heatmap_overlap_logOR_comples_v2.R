heatmap_overlap_log2OR_complex <- function(
    ...
    row_mapper = NULL,
    col_mapper = NULL,
    col_order_original = NULL,   # <=== NOWY ARGUMENT
  ...
  if (!is.null(col_order_original)) {
    original_to_full <- setNames(col_labels, mapped_cols)
    desired <- original_to_full[col_order_original]
    
    if (any(is.na(desired))) {
      stop("Not all names from col_order_original found after mapping:\n",
           paste(col_order_original[is.na(desired)], collapse=", "))
    }
    
    log2or  <- log2or[, desired, drop=FALSE]
    overlap <- overlap[, desired, drop=FALSE]
    pval    <- pval[, desired, drop=FALSE]
  }
