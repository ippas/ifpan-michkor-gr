...
RE_ERROR = re.compile(
    r"(Traceback \(most recent call last\):.*|ValueError:.*|Exception:.*|ERROR:.*|Error:.*)",
    re.MULTILINE | re.DOTALL
)
...
def basename_without_sumstats(path_str: Optional[str]) -> Optional[str]:
    ...
    for suffix in [".sumstats.gz", ".sumstats", ".gz", ".tsv", ".txt"]:
...
def parse_ldsc_log(log_path: Path) -> Dict[str, Optional[object]]:
    ...
    if row["analysis_end"] is not None and row["rg"] is not None:
...
def collect_log_files(input_dir: Path, recursive: bool = True, sort_files: bool = True) -> List[Path]:
    if recursive:
