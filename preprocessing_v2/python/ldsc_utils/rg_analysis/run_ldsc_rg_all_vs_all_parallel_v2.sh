...
run_pair() {
    ...
    if [[ -f "${out_prefix}.log" ]]; then
        ...
        echo "[${done_count}/${TOTAL_PAIRS} | ${percent}% | left:${left_count} | ok:${ok_count} fail:${fail_count} skip:${skip_count}] SKIP ${trait1} vs ${trait2}"
        return 0
    fi
    ...
    if [[ "$exit_code" -eq 0 ]]; then
        ...
        echo "[${done_count}/${TOTAL_PAIRS} | ${percent}% | left:${left_count} | ok:${ok_count} fail:${fail_count} skip:${skip_count}] OK   ${trait1} vs ${trait2} ($(format_seconds "$runtime"))"
    else
        {
            echo "status=failed"
            echo "trait1=${trait1}"
        ...
        echo "[${done_count}/${TOTAL_PAIRS} | ${percent}% | left:${left_count} | ok:${ok_count} fail:${fail_count} skip:${skip_count}] FAIL ${trait1} vs ${trait2} ($(format_seconds "$runtime"))"
    fi
}
