...
# ---- plots ----
p1 <- ggplot(df_plot, aes(x = n_norm_phenotypes_01, y = icdcategory)) +
...
p2 <- ggplot(df_plot, aes(x = n_norm_genes_01, y = icdcategory)) +
  geom_col(width = bar_width, fill = bar_fill, color = bar_edge, size = bar_lwd, alpha = 0.7) +
  x_scale +
...
p3 <- ggplot(df_plot, aes(x = n_norm_source_01, y = icdcategory)) +
  geom_col(width = bar_width, fill = bar_fill, color = bar_edge, size = bar_lwd, alpha = 0.7) +
  x_scale +
  labs(x = "0–1", y = NULL, title = title_src) +
  theme_classic() +
  theme_black_text +
  theme(
    axis.text.y  = element_blank(),
...
p4 <- ggplot(df_plot, aes(x = score, y = icdcategory)) +
...
ggplot(df_plot, aes(x = n_norm_genes, y = icdcategory)) +
...
out_file <- "summaryAssociations_grSystemic_v1_03.02.2026.svg"
...
svg(
  ...
  width = 12,   # cale — dostosuj pod journal
  height = 6,   # cale
  pointsize = 10
)
...
gene_list_filtered[grepl("F0x|Alzheim",  names(gene_list_filtered))] %>% unname %>% unlist %>% unique %>% length()

gene_list_filtered[grepl("F3x",  names(gene_list_filtered))] %>% unname %>% unlist %>% unique %>% length()
