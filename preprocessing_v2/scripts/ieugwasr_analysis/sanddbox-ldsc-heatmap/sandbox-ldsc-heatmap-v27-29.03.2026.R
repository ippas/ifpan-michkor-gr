create_rg_heatmap <- function(
  ...
  if (cluster) {
    ...
    if (isTRUE(use_row_cluster_gaps)) {
      row_split <- row_dend_k
    } else {
      row_split <- NULL
    }
    
    if (isTRUE(use_col_cluster_gaps)) {
      col_split <- col_dend_k
    } else {
      col_split <- NULL
    }
  ...
  draw_heatmap <- function() {
    ...
    if (isTRUE(use_row_cluster_gaps) || isTRUE(use_col_cluster_gaps)) {
      row_ord <- row_order(ht_drawn)
      col_ord <- column_order(ht_drawn)
      
      n_row_slices <- if (is.list(row_ord)) length(row_ord) else 1L
      n_col_slices <- if (is.list(col_ord)) length(col_ord) else 1L
      
      boundary_color <- if (isTRUE(use_row_cluster_gaps) && isTRUE(use_col_cluster_gaps)) {
        row_gap_color
      } else if (isTRUE(use_row_cluster_gaps)) {
        row_gap_color
      } else if (isTRUE(use_col_cluster_gaps)) {
        col_gap_color
      } else {
        "black"
      }
      
      boundary_lwd <- max(row_gap_width_mm, col_gap_width_mm) * 2.2
      
      for (i in seq_len(n_row_slices)) {
        for (j in seq_len(n_col_slices)) {
