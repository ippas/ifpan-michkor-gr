create_rg_heatmap <- function(
  ...
  convert_png_dimension <- function(value, unit, res) {
    if (unit == "px") {
  ...
  if (!is.null(output_png)) {
    ...
    if (is.null(png_width_parsed) && is.null(png_height_parsed)) {
      ...
      width_conv <- convert_png_dimension(png_width_parsed$value, png_width_parsed$unit, png_res)
      png_args$width <- width_conv$value
      png_args$height <- 10
      png_args$units <- width_conv$units
    } else if (is.null(png_width_parsed) && !is.null(png_height_parsed)) {
      height_conv <- convert_png_dimension(png_height_parsed$value, png_height_parsed$unit, png_res)
      png_args$width <- 10
