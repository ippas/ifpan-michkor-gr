create_rg_heatmap <- function(
  ...
  if (!all(label_fields %in% available_label_fields)) {
    stop(
      "Nieprawidłowe wartości w 'label_fields'. Dostępne opcje: ",
  ...
  # 6. Etykiety
  # =========================================
  label_metadata <- metadata_ieu_EURsampleSize10000 %>%
  ...
  format_label_value <- function(field_name, value) {
    ...
    if (field_name == "subcategory") {
      return(paste0("subcategory: ", value))
    }
    ...
    if (field_name == "sample_size") {
      return(paste0("sample size: ", value))
    }
...
create_rg_heatmap(
  ...
  rg_text_y_offset_mm = 1.0,
  star_text_y_offset_mm = -2.5,
  label_fields = c("id", "trait", "subcategory", "nsnp", "sample_size"),
