...
## - nuclearReceptors_GO0004879 (character vector of TF names)
## - multi_downloadMedianSubtissuesGTEx_tissueSummaryExpression() (your function)
##
## This script will generate:
## - top10_per_signature
## - TF_expression_GTEx
## - hm_df, TF_expr_flags, dots_df
## - p_heat_cat
...
## ----------------------------
## 3b) OPTIONAL: alias fixes (your mapping)
## ----------------------------
...
## ----------------------------
## 4) Download GTEx summary for these TFs
## ----------------------------
...
## ----------------------------
## 5) Heatmap dataframe (complete grid + bins + coords)
## ----------------------------
...
## ----------------------------
## 6) GTEx expression flags per TF (Brain/Lung/Blood)
## ----------------------------
## Expecting columns: query_gene_symbol, tissue, median_max
...
## 7) Dots dataframe
##   * global_* : 3 dots
##   * brain/lung/blood_* : 1 dot only (matching tissue)
## ----------------------------
dots_df <- hm_df %>%
  ...
  mutate(
    ...
    ## offsets: global = 3 dots; tissue-specific = centered dot
    x_offset = case_when(
...
p_heat_cat <- ggplot(hm_df, aes(x = signature, y = y_num, fill = fdr_bin)) +
  ...
  ## dots: hollow background (always drawn)
  geom_point(
    data = dots_df,
    inherit.aes = FALSE,
    aes(x = x_dot, y = y_dot),
  ...
  ## dots: expressed (colored overlay)
  geom_point(
