...
gene_summary_list$all$summary_all
gene_summary_list$long_time$summary_all

gene_summary_list$n10_short_time
...
tibble::tibble(
  ...
  { pie(
    ...
    main = "Distribution of genes by frequency (mirrored, rotated 90°)",
    col = rep("lightgrey", nrow(.)),  # jasno szary kolor
    border = "black",                 # czarna obramówka
    lwd = 2,                          # grubość linii
    init.angle = 90,                  # obrót o 90°
    clockwise = FALSE                 # lustrzane odbicie
  ) }